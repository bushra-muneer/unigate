...
from unigate import crud
from unigate.core.database import get_auth_session, get_session
from unigate.core.database import uniStub_engine as engine
from unigate.core.security import get_password_hash
from unigate.enums import GroupType
from unigate.models.course import Course
from unigate.models.exam import Exam
...
courses = {
    CourseCreate(name="Test Course"): [datetime(2025, 1, 1, tzinfo=pytz.utc), datetime(2025, 1, 25, tzinfo=pytz.utc)],
    CourseCreate(name="Capstone"): [datetime(2025, 6, 6, tzinfo=pytz.utc), datetime(2025, 7, 13, tzinfo=pytz.utc)],
    CourseCreate(name="Distributed Systems"): [datetime(2025, 1, 11, tzinfo=pytz.utc), datetime(2025, 2, 8, tzinfo=pytz.utc)],
    CourseCreate(name="Binary Analysis and secure coding"): [datetime(2025, 1, 8, tzinfo=pytz.utc), datetime(2025, 2, 10, tzinfo=pytz.utc)],
    CourseCreate(name="Decentralized Systems"): [datetime(2025, 1, 14, tzinfo=pytz.utc), datetime(2025, 2, 13, tzinfo=pytz.utc)],
    CourseCreate(name="High Performance Computing"): [datetime(2025, 1, 21, tzinfo=pytz.utc), datetime(2025, 2, 18, tzinfo=pytz.utc)],
    CourseCreate(name="Machine Learning"): [datetime(2025, 1, 20, tzinfo=pytz.utc), datetime(2025, 2, 19, tzinfo=pytz.utc)],
    CourseCreate(name="Network Security"): [datetime(2025, 1, 27, tzinfo=pytz.utc), datetime(2025, 2, 20, tzinfo=pytz.utc)],
    CourseCreate(name="Digital Forensics"): [datetime(2025, 1, 28, tzinfo=pytz.utc), datetime(2025, 2, 21, tzinfo=pytz.utc)],
    CourseCreate(name="Virtualization and Cloud Computing"): [datetime(2025, 2, 4, tzinfo=pytz.utc), datetime(2025, 2, 26, tzinfo=pytz.utc)],
    CourseCreate(name="Data Protection and Privacy"): [datetime(2025, 2, 11, tzinfo=pytz.utc), datetime(2025, 2, 27, tzinfo=pytz.utc)],
}
...
def seed_auth() -> None:
    ...
    for i, user in enumerate(users):
        ...
        if i % 3 == 0:
            for course in created_courses:
                session.add(Teach(professor_id=created_user.id, course_id=course.id))
    ...
    for student in students:
        ...
        if student.number != 4820312:
            for group in groups:
                ...
                group_obj.students.append(current_student)
                group_obj.super_students.append(current_student)
