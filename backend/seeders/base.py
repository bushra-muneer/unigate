...
#         exams = [
#             Exam(course_id=course.id, date=date(2025, 4, 15)),
#             Exam(course_id=course.id, date=date(2025, 5, 16)),
#             Exam(course_id=course.id, date=date(2025, 6, 16)),
#         ]
#         for exam in exams:
#             session.add(exam)
#         session.commit()
...
#                 session.add(
#                     ExamResult(
#                         student_id=student.id, exam_id=exam.id, passed=i % 2 == 0
#                     )
#                 )
#         session.commit()
...
from unigate.utils.exam_sync import sync_exam_results
from unigate import crud
from unigate.core.database import get_auth_session, get_session, uniStub_engine as engine
from unigate.core.security import get_password_hash
from unigate.enums import GroupType
from unigate.models.course import Course
from unigate.models.exam import Exam
...
students = [
    StudentCreate(number=1234567, email="s1234567@studenti.unige.it", name="Test Name", surname="Test Surname"),
    StudentCreate(number=4891185, email="s4891185@studenti.unige.it", name="Fabio", surname="Fontana"),
    StudentCreate(number=4989646, email="s4989646@studenti.unige.it", name="Lorenzo", surname="Foschi"),
    StudentCreate(number=5806782, email="s5806782@studenti.unige.it", name="Mimmo", surname="Torabi"),
    StudentCreate(number=6015033, email="s6015033@studenti.unige.it", name="Musse", surname="Gher"),
    StudentCreate(number=4820312, email="s4820312@studenti.unige.it", name="Giovanni", surname="Bosi"),
    StudentCreate(number=5475593, email="s5475593@studenti.unige.it", name="Forough", surname="Majidi"),
    StudentCreate(number=4878744, email="s4878744@studenti.unige.it", name="Michele", surname="Frattini"),
]

professors = [
    AuthUserCreate(number=0, email="test@unige.it", name="Test Name", surname="Test Surname", hashed_password=get_password_hash("testpassword")),
    AuthUserCreate(number=1000000, email="marina.ribaudo@unige.it", name="Marina", surname="Ribaudo", hashed_password=get_password_hash("testpassword")),
    AuthUserCreate(number=2000000, email="maura.cerioli@unige.it", name="Maura", surname="Cerioli", hashed_password=get_password_hash("testpassword")),
    AuthUserCreate(number=3000000, email="matteo.dellamico@unige.it", name="Matteo", surname="Dell'Amico", hashed_password=get_password_hash("testpassword")),
    AuthUserCreate(number=4000000, email="s6180175@studenti.unige.it", name="Bushra", surname="Muneer", hashed_password=get_password_hash("testpassword")),
]

groups = [
    GroupCreate(name="Test Public Group", description="This is a test group", category="Test", type=GroupType.PUBLIC, course_name="Test Course", date=datetime(2025, 1, 1, tzinfo=pytz.utc), exam_date=datetime(2025, 1, 1, tzinfo=pytz.utc)),
    GroupCreate(name="Test Public Group", description="This is a test group", category="Test", type=GroupType.PUBLIC, course_name="Test Course", date=datetime(2025, 2, 2, tzinfo=pytz.utc), exam_date=datetime(2025, 2, 2, tzinfo=pytz.utc)),
    GroupCreate(name="Test Private Group", description="This is a test group", category="Test", type=GroupType.PRIVATE, course_name="Test Course", date=datetime(2025, 1, 1, tzinfo=pytz.utc), exam_date=datetime(2025, 1, 1, tzinfo=pytz.utc)),
]
...
users = professors + [
    AuthUserCreate.model_validate(student, update={"hashed_password": get_password_hash("testpassword")})
    for student in students
]
...
def seed_auth() -> None:
    ...
    for user in users:
        ...
        if user in professors:
            for course_name, details in courses.items():
                if user in details["professors"]:
                    session.add(Teach(professor_id=created_user.id, course_id=created_courses[course_name].id))
...
def seed_UniStub() -> None:
    with Session(engine) as session:  # type: ignore
        ...
        # Add students (skip Test)
        students = [
            Student(number=4891185, name="Fabio", surname="Fontana", email="s4891185@studenti.unige.it"),
            Student(number=4989646, name="Lorenzo", surname="Foschi", email="s4989646@studenti.unige.it"),
        ]
        for student in students:
            session.add(student)
        session.commit()

        # Add courses (skip Test)
        courses = [
        ...
        # Refresh courses to get their IDs
        session.refresh(courses[0])
        session.refresh(courses[1])

        # Map course names to exam dates
        course_exam_map = {
        ...
        for course in courses:
            for exam_date in course_exam_map[course.name]:
                exam = Exam(course_id=course.id, date=exam_date)
                session.add(exam)
                session.commit()
