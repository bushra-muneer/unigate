        ...
        <div v-for="status in groupByStatus(course.groups).order" :key="status">
          <div v-if="groupByStatus(course.groups).map[status]">
            ...
            >
              ...
              {{ status }} ({{
                groupByStatus(course.groups).map[status].length
              }}
              Groups)
            </div>
            <div
            ...
            >
              <div
                v-for="(dateGroups, date) in groupByDate(
                  groupByStatus(course.groups).map[status],
                )"
                :key="date"
                class="mb-2"
              >
                ...
                <div
                  v-show="isStatusExpanded(status, date)"
                  class="flex flex-col"
                >
                  <div class="flex space-x-4 overflow-x-auto">
                    <GroupCard
                      v-for="group in getSortedGroups(
                        ...
                        date,
                        dateGroups,
                      ).slice(0, getCurrentLimit(status, date))"
                      :key="group.id"
                      :group="group"
                      class="min-w-[250px] max-w-[250px] flex-shrink-0"
                    />
